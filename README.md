...
운영진이 함께 사용하는 유저 시트, 개인 운영 메모, 참여 대조, 실시간 공동 편집과 결과 전달도 지원합니다.

## 주요 기능

- Discord 채팅 명단 파싱 및 기존 참가 명단과 변경 내용 비교
- 유저 시트 검색, 수동 입력과 공지 참여자 대조
- Discord 고유 ID를 기준으로 한 참가자 식별과 기존 유저 재연결
...
- 운영진 공유 유저 시트와 전용 관리 페이지
- 10자리 코드 기반 실시간 공동 편집과 읽기 전용 결과 전달
- 공동 작업자 Discord 프로필과 최근 수정자 표시
- 작성자만 보는 개인 운영 메모
- 단계별 사용 가이드
...
테스터1#11853 다5/다1/다5
테스터2#38848 다3/마4/다4
테스터3#34981 미배치/플2/골1
테스터4#31207 그5!/마1/마4
```

역할 순서는 항상 `탱커 / 딜러 / 힐러`입니다.
세 역할 중 최소 두 역할의 정식 티어가 필요하며, 나머지 한 역할은 생략하거나 `미배치`로 적을 수 있습니다.
...
원격 데이터 모드에서는 가져오기 전에 각 참가자를 Discord ID, BattleTag와 표시 이름 순으로
기존 유저 시트와 대조합니다. 신규 유저와 중복 후보는 17~20자리 Discord ID를 확인해야 하며,
기존 유저의 티어를 갱신할지 참가자별로 선택할 수 있습니다. 로컬 전용 모드에서는 이 검토를
명단에만 적용하고 유저 시트를 수정하지 않습니다.

### 2. 수동으로 참가자 추가하기

`수동 입력`에서 Discord 이름, BattleTag, 역할별 티어와 선호·비선호 역할을 선택합니다.
저장된 유저는 `유저 시트` 입력 방식에서 이름·Discord ID·BattleTag로 검색해 바로 추가할 수 있습니다.
...
참가자 10명이 준비되면 `팀 자동 배정` 버튼이 활성화됩니다. 알고리즘은 다음 조건을 우선순위대로
비교한 뒤 양 팀의 총점과 역할별 점수 차이를 줄입니다.

1. 선호 역할 위반 최소화 (`선호 무시` 옵션 사용 시 제외)
2. 탱커 티어 차이 안전 범위 우선
3. 비선호 역할 배정 최소화
4. 미배치 역할 배정 최소화
5. 역할별 격차와 팀 내부 편차 최소화

결과가 마음에 들지 않으면 다른 대안을 선택하거나, 두 선수를 차례로 눌러 직접 교체할 수 있습니다.

### 5. 결과 공유하기

`이미지로 복사`를 누른 뒤 Discord 채팅에 바로 붙여넣습니다. 유저 시트의 특이사항은 시트 UUID,
Discord ID, 유일한 BattleTag 순으로 참가자와 연결되어 화면에 표시되지만 복사 이미지에서는 제외됩니다.

### 6. 다른 관리자와 공유하기

대진표의 `공유` 영역은 목적에 따라 두 방식으로 나뉩니다.

| 방식 | 용도 | 동작 |
| --- | --- | --- |
| 함께 편집 | 여러 관리자가 같은 명단과 팀 배치를 수정 | 변경 내용을 양방향 동기화 |
| 결과 전달 | 확정한 결과를 다른 관리자에게 전달 | 불러온 시점의 결과를 읽기 전용으로 복원 |

두 방식 모두 10자리 코드와 24시간 만료 시간을 사용합니다. `함께 편집`은 참가자와 대기열을 포함해
최대 50명의 현재 명단을 공유하며 모든 인원이 유저 시트의 Discord ID와 연결되어 있어야 합니다.
`결과 전달`은 Discord ID가 연결된 10명의 최신 팀 배정 결과가 필요합니다. `함께 편집` 코드는 관리자별 브라우저에 24시간
보존되어 새로고침 뒤 자동 재연결됩니다. 변경 직후에는 500ms, 30초 동안 변경이 없으면 1.5초
간격으로 확인하고, 탭으로 돌아오면 즉시 최신 상태를 가져옵니다.

연결 중인 관리자의 Discord 프로필과 이름, 최근 수정자·수정 종류·시각은 관리자 전역 상단 상태 바와
팀 결과 영역에서 확인할 수 있습니다. 동시에 수정해 revision 충돌이 발생하면 서버의 최신 상태를 먼저
반영하고 필요한 변경을 다시 적용하도록 안내합니다.
...
상만	123456789012345678	Player#1234	다3	플2	플3	운영진 공용 참고
```

- Discord ID는 필수이며 유저 연결과 중복 판정의 가장 안정적인 기준입니다.
- 같은 BattleTag라도 서로 다른 Discord ID가 확인되면 별도 유저로 저장할 수 있습니다.
- 역할 티어의 `!`, `?`, `★` 기호는 시트에 저장할 때 제거합니다.
- 사이드바의 `유저 시트` 또는 `/user-sheet`에서 검색·상세 확인·수정·삭제를 수행합니다.
- 상세 화면의 `바로 수정`으로 현재 화면에서 한 유저만 수정할 수 있습니다.
- 여러 유저는 `전체 편집`에서 표 형태로 수정하거나 Google Sheets의 기존 6열 또는 Discord ID를
  포함한 7열 데이터를 붙여넣을 수 있습니다. 저장 전 모든 행에 Discord ID가 필요합니다.
- 전체 편집의 `Discord 명단 가져오기`는 Discord ID를 우선해 기존 유저를 연결하고 신규 유저를 추가합니다.
  기존 특이사항은 보존하며 실제 저장 전 편집 표에서 변경 내용을 확인합니다.
- 시트가 열려 있으면 1분마다 최신 데이터를 확인하며, 창으로 돌아오거나 새로고침 버튼을 누르면
  즉시 갱신합니다.
- 시트 전용 URL을 새로고침해도 같은 페이지가 유지됩니다.
...
- Discord 사용자명, 표시 이름과 프로필 이미지 주소는 관리자 계정 표시를 위해 로그인 세션에 포함되며 세션과 함께 만료됩니다.
...
- 실시간 공동 작업과 읽기 전용 결과 코드는 Redis에 24시간 저장되며 Discord ID와 팀 위치만 공유합니다.
...
- Node.js 24
- pnpm 11.9
- 저장소에 포함된 Vercel CLI 58.9 이상(전역 CLI는 59.17 이상 권장)
...
`pnpm check`는 타입 검사, 린트, 테스트, 도메인 경계 검사와 프로덕션 빌드를 순서대로 실행합니다.
...
├── match-shares/         # 실시간 공동 편집·읽기 전용 결과 공유 API
├── notes/                # 개인 운영 메모 API
├── public/               # 공개 참여 API
├── scrims/               # 내전 운영 API
